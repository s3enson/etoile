...
function LengthSelector({
	...
	const title = selectedType === "bracelet" ? "DŁUGOŚĆ BRANSOLETKI" : "DŁUGOŚĆ NASZYJNIKA";

	return (
		<section className="length-section">
			<div className="length-header" onClick={() => setIsLengthOpen((prev) => !prev)}>
				<span className="length-title">{title}</span>

				<button type="button" className="length-arrow-button" aria-label="Rozwiń lub zwiń">
					<svg
						width="16"
						height="16"
						viewBox="0 0 24 24"
						fill="none"
						className={`length-arrow ${isLengthOpen ? "" : "length-arrow-closed"}`}>
						<path
							d="M6 15L12 9L18 15"
							stroke="currentColor"
							strokeWidth="2"
